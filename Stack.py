...
for train, test in skf.split(X,y):
   ...
   for modelname in modelist:
       ...
       oof_train_ ,oof_test_= get_oof(clf=clf_first,n_folds=5,X_train=X[train],y_train=y[train],X_test=X[test])
   ...
   clf_second1 = SVC(kernel='rbf', C=6, gamma=0.005, probability=True)
   #clf_second1 = xgb.XGBClassifier( max_depth=15, learning_rate=0.01,
                 #n_estimators=500, silent=True,
                 #max_delta_step=0, subsample=1, colsample_bytree=1, colsample_bylevel=1,
                 #reg_alpha=1, reg_lambda=2, scale_pos_weight=1,
                 #base_score=0.5)
   #clf_second1 = RandomForestClassifier(n_estimators=500)
   #clf_second1 = GradientBoostingClassifier(n_estimators=1200,max_depth=6,subsample=0.7,learning_rate=0.01)
